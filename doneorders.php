...
if(isset($_GET['delete'])){
 ...
 if(isset($_GET['report'])){
    ...
    $d="SELECT  SUM(total_price) from `order`";
    $qv = mysqli_query($conn, $d);
    $ro=mysqli_fetch_assoc($qv);
    
    $f="SELECT  SUM(total_products) from `order`";
    ...
    if(mysqli_num_rows($qr)>0){
      ...
      $html='<table border="1">';
      $html.='<tr><td colspan="5"><center> User Order Details</center></td></tr><tr><td>User Name</td><td>Food Name</td><td>Product Quantity</td><td>Total Ammount</td><td>Order Date</td></tr>';
      while($row=mysqli_fetch_assoc($qr))
      {
          $html.='<tr><td>'.$row['name'].'</td><td>'.$row['pr_name'].'</td><td>'.$row['total_products'].'</td><td>'.$row['total_price'].'</td><td>'.$row['order_date'].'</td></tr>';
      }
    $html.='<tr><td colspan="2">Total Sales</td><td>'. $ry['SUM(total_products)'].'</td><td>'. $ro['SUM(total_price)'].'</td></tr>';
...
<h1 class="heading">Order Prepared</h1>
   <table>
...
<th>Address</th>
      ...
      <tbody>
         <?php   
            $q = "SELECT * FROM `order` WHERE `status`='done' ORDER BY order_date DESC";
            $qr = mysqli_query($conn, $q);

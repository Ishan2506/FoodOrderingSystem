...
$cart_quer = mysqli_query($conn, "SELECT * FROM `cart` WHERE uid = '$user_id'");
if(mysqli_num_rows($cart_quer) > 0){
   while($pro_item = mysqli_fetch_assoc($cart_quer)){
      
         $pro_name = $pro_item['name'];
         $pro_price = ($pro_item['price'] * $pro_item['quantity']);
         $pro_quantity=$pro_item['quantity'];
         $pro_id=$pro_item['p_id'];
         $pro_cat=$pro_item['cat'];

         
$detail_quer = mysqli_query($conn, "INSERT INTO `sales`(name,price,quant,p_id,cat) VALUES('$pro_name','$pro_price','$pro_quantity','$pro_id','$pro_cat')") or die('query failed');
...
if(isset($_POST['order_btn'])){
   ...
   $cart_query = mysqli_query($conn, "SELECT * FROM `cart` WHERE uid = '$user_id'");
   $price_total = 0;
   $total_product=0;
   $ppname="";
   if(mysqli_num_rows($cart_query) > 0){
      while($product_item = mysqli_fetch_assoc($cart_query)){
         ...
         for ($i=0; $i <1; $i++) { 
            ...
            $n= $product_item['p_id'];
            $up = mysqli_query($conn, "UPDATE `products` SET p_quantity= p_quantity-$q  WHERE id=$n");   # code...
         }
   ...
   if($cart_query && $detail_query){
      ...
      $body = 'Thnaks For order in DID Cafe.<br>
              <br>
              Your Name:- '.$name.'<br>
              Your Number:- '.$number.'<br>
              Your Address is:- '.$flat.','.$street.','.$city.','.$pin_code.'<br>
              Your Total Amount is:- '.$price_total.'<br>
              Your Total Product is:- '.$total_product.'<br>
      ...
      // $mail->addAddress('ishantrivedi092@gmail.com');
...
<body>
  ...
  <section class="home-section">
   ...
   ?> 
      ...
      <?php
      include 'config.php';
      $select_rows = mysqli_query($conn, "SELECT * FROM `cart` WHERE uid = '$user_id'") or die('query failed');
...
<section class="checkout-form">
   ...
   <div class="display-order">
      <?php
         $select_cart = mysqli_query($conn, "SELECT * FROM `cart`  WHERE uid = '$user_id'");
         $total = 0;
         $grand_total = 0;
         $sd="";
   ...
   ?>
      ...
      <div class="flex">
         ...
         <div class="inputBox">
            <span>city</span>
            <input type="text" placeholder="e.g. mumbai" name="city" value="<?php echo $fetch_edit['city']; ?>"  required>
         </div>
        
         <div class="inputBox">
            <span>pin code</span>
            <input type="text" placeholder="e.g. 123456" name="pin_code" value="<?php echo $fetch_edit['pincode']; ?>"  required>
         </div>

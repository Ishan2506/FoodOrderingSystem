...
if(isset($_GET['delete'])){
 ...
 if(isset($_GET['prepered'])){
  $prepered_id = $_GET['prepered'];

  // $approve_query = mysqli_query($conn, "DELETE status FROM `order` WHERE id = $approve_id") or die('query failed');
  $prepered_query = mysqli_query($conn, "UPDATE `order` SET status='done' WHERE id =$prepered_id") or die('query failed');
...
<div class="sidebar">
    <div class="logo-details">
      ...
      <ul class="nav-links">
        ...
        <li>
          <a href="ordertak.php" class="active">
...
<h1 class="heading">Order Pending</h1>
   <table>
      ...
      <tbody>
         <?php   
            $q = "SELECT * FROM `order` WHERE `status`='disapprove'";
      ...
      ?>
           ...
           <td><?php echo $row['name']; ?></td>
            ...
            <td><?php for($i = 0; $i < count($a); $i++){ echo "<ul'><li>".$a[$i]."</li></ul>";} ?></td>
             <td><?php echo $row['flat']." , ".$row['street']." , ".$row['city']; ?></td>
           
            <td>
               <a href="ordertak.php?delete=<?php echo $row['id']; ?>" class="delete-btn" onclick="return confirm('are your sure you want to delete this?');"> <i class="fas fa-trash"></i> Cancel </a>
               <a href="ordertak.php?apprrove=<?php echo $row['id']; ?>" class="aa-btn"> <i class="fas fa-edit"></i> Apprrove </a>
            </td>
         </tr>
         <?php
            };    
...
<h1 class="heading">Approved Orders</h1>
   <table>
      ...
      <tbody>
         <?php   
            $q = "SELECT * FROM `order` WHERE `status`='approve'";
            $qr = mysqli_query($conn, $q);
      ...
      ?>
            ...
            <td>
               <a href="ordertak.php?delete=<?php echo $row['id']; ?>" class="delete-btn" onclick="return confirm('are your sure you want to delete this?');"> <i class="fas fa-trash"></i> Cancel </a>
               <a href="ordertak.php?prepered=<?php echo $row['id']; ?>" class="aa-btn"> <i class="fas fa-edit"></i> Prepared </a>
            </td>
         </tr>
         <?php
            };    
            }else{
               echo "<div class='empty'>A single order has not been accepted yet</div>";
            };
...
<section class="display-product-table">
<h1 class="heading">Order Canceled</h1>
   <table>
      ...
      <tbody>
         <?php   
            $q = "SELECT * FROM `order` WHERE `status`='cancel'";
            $qr = mysqli_query($conn, $q);
      ...
      ?>
         ...
         <?php
               ...
               echo "<div class='empty'> Not any Order Canceled </div>";
            };
